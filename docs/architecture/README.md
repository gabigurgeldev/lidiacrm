...
| `disparo-em-massa.architecture.json` | disparo em massa — 21 peças, 32 arestas, 7 faixas, mais quatro não-ligações declaradas; por que o anti-ban é IMPORTADO e não reimplementado (o operador só consegue ir mais devagar), por que o desfecho vem da linha de `messages` e nunca da ausência de exceção, e a cláusula de exclusividade por número que o claim das peças irmãs não tem — sem ela duas campanhas do mesmo chip respeitam o throttle cada uma por si e o número real sai ao dobro |
| `flow-builder-ia.architecture.json` | criar fluxo com IA, dentro do editor — 14 peças, 21 arestas, 6 faixas, mais três não-ligações declaradas; o schema de geração nasce do `registry` (nunca copiado à mão), a posição vem de BFS puro porque a IA não sabe desenhar layout, a geração NUNCA grava `flows.draft_graph` (só o "Salvar rascunho" de sempre), e o modelo é escolhido pelo admin da EMPRESA — não um mecanismo novo de plataforma |
