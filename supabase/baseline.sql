...
-- ---- Flow Engine: da frente ÚNICA às múltiplas frentes (migration 0207) ----
--
-- Conteúdo idêntico à migration 0207. Auto-curativo nos dois modos: no install
-- fresco o backfill não encontra execução nenhuma e é no-op; no `update.sh` de
-- um clone com fluxo rodando, ele dá um frame a cada execução viva ANTES de o
-- motor novo passar a enxergar só frames.
...
-- ---- cascata de LGPD alcança o motor de fluxos (migration 0208) ----
--
-- `flow_executions.input` guarda o texto que o cliente mandou (o payload do
-- evento que armou a execução). Sem este passo, anonimizar devolve sucesso e a
-- frase segue legível. Idempotente: `create or replace`.
...
-- ---- o placar do bloco que divide os caminhos (migration 0215) ----
...
-- ---- fluxos ao vivo na publicação realtime (migration 0214) ----
...
-- ---- vínculo com o Back Office de afiliados (migration 0216) ----
