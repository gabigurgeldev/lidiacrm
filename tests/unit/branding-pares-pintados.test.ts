...
describe("controle positivo — o produto sem marca não pode se mexer", () => {
  it("a semente do produto reproduz, pintada, os números do design system", () => {
    // `#13731b` é a semente do próprio produto: ela não desloca nada, e os pares
    // pintados têm que dar o que o `globals.css` sempre deu. Se estes números
    // mudarem, o conserto vazou para quem não pediu.
    const cor = corDe("#13731b");
    expect(cor.derivada?.claro.deslocamento).toBe(0);
    expect(cor.derivada?.escuro.deslocamento).toBe(0);

    const p = pintadosDaSemente("#13731b");
    // Claro: os dois números que `contraste.ts` documenta como medidos à mão.
    expect(foco(p.claro, "--color-bg")).toBeCloseTo(4.05, 2);
    expect(foco(p.claro, "--color-surface-elevated")).toBeCloseTo(3.64, 2);
    // Escuro: na rampa DERIVADA da semente. A diferença de centésimo contra os
    // literais do `globals.css` é o arredondamento de Δ ≤ 2/255 por canal com
    // que a rampa reproduz a paleta.
    expect(foco(p.escuro, "--color-bg")).toBeCloseTo(6.65, 2);
    expect(foco(p.escuro, "--color-surface-elevated")).toBeCloseTo(5.7, 2);
    // No escuro o anel NÃO fica apertado contra as bases: quem aperta é o
    // `-soft` COMPOSTO. É este o par que a prova em tela reportou como o mais
    // apertado do tema escuro, e não `foco × --color-bg`.
    expect(foco(p.escuro, "--color-accent-soft@--color-surface")).toBeCloseTo(4.92, 2);
...
describe("a navy #0f172a — o defeito que a prova em tela achou", () => {
  it("os quatro números do anel de foco, agora acima do piso", () => {
    // ANTES (medido no browser, servidor de dev na 3111, na paleta Sage): claro
    ...
    // escuro anda -1, então o anel passou a pintar o stop 300 da rampa da marca.
    //
    // Os números subiram um pouco com a paleta Gestalt e a causa é a SUPERFÍCIE,
    // não o anel: o claro mede contra `#ffffff` (era `#faf9f6`) e o escuro contra
    // `#0c0e10` (era `#161510`). A navy pintada é a mesma; o fundo é que ficou
    // mais extremo nas duas pontas. O que o caso guarda — o anel do tema escuro
    // acima do piso — segue sendo a asserção do laço abaixo.
    const p = pintadosDaSemente("#0f172a");
    expect(foco(p.claro, "--color-bg")).toBeCloseTo(11.33, 2);
    expect(foco(p.claro, "--color-surface-elevated")).toBeCloseTo(10.19, 2);
    expect(foco(p.escuro, "--color-bg")).toBeCloseTo(5.59, 2);
    expect(foco(p.escuro, "--color-surface-elevated")).toBeCloseTo(4.79, 2);
